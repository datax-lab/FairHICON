...
class Classifier(nn.Module):
    def __init__(self, net_hparams):        
        super(Classifier, self).__init__()        
        ### net_hparams
        ### 0-input_nodes, 1-hidden_nodes, 2-output_nodes, 3-initializer, 4-activation, 5-dropout
        self.clf_layer1 = nn.Linear(net_hparams[0], net_hparams[1][0])
        init_parameters(self.clf_layer1, net_hparams[3])
        self.clf_bn1 = nn.BatchNorm1d(net_hparams[1][0], momentum=0.01)
        
        self.clf_layer2 = nn.Linear(net_hparams[1][0], net_hparams[1][1])
        init_parameters(self.clf_layer2, net_hparams[3])
        self.clf_bn2 = nn.BatchNorm1d(net_hparams[1][1], momentum=0.01)
        
        
        self.clf_layer3 = nn.Linear(net_hparams[1][1], net_hparams[1][2])
        init_parameters(self.clf_layer3, net_hparams[3])
        self.clf_bn3 = nn.BatchNorm1d(net_hparams[1][2], momentum=0.01)
        
        self.clf_layer4 = nn.Linear(net_hparams[1][2], 1)
        init_parameters(self.clf_layer4, net_hparams[3])
        
        # Activation functions
        act = net_hparams[4]
        if act == "sigmoid":
            self.act1 = nn.Sigmoid();  self.act2 = nn.Sigmoid();  self.act3 = nn.Sigmoid()
        elif act == "tanh":
            self.act1 = nn.Tanh();     self.act2 = nn.Tanh();     self.act3 = nn.Tanh()
        elif act == "relu":
            self.act1 = nn.ReLU();     self.act2 = nn.ReLU();     self.act3 = nn.ReLU()
        elif act == "lkrelu":
            self.act1 = nn.LeakyReLU(); self.act2 = nn.LeakyReLU(); self.act3 = nn.LeakyReLU()
        else:
            raise ValueError(f"Unknown activation '{act}'")
    ...
    def forward(self, x):
        # Layer1 → BN → Activation → (Optionally Dropout)
        x = self.clf_bn1(self.clf_layer1(x))
        x = self.act1(x)
        if self.on_Dropout:
            x = self.dropout(x)

        # Layer2 → BN → Activation → (Optionally Dropout)
        x = self.clf_bn2(self.clf_layer2(x))
        x = self.act2(x)
        if self.on_Dropout:
            x = self.dropout(x)
            
        # Layer3 → BN → Activation → (Optionally Dropout)
        x = self.clf_bn3(self.clf_layer3(x))
        x = self.act3(x)
        if self.on_Dropout:
            x = self.dropout(x)
            
        pred = self.output_activation(self.clf_layer4(x))
