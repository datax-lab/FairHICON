...
def plot_loss_plots(data, ver, date, num, experiment, train_loss_values, valid_loss_values):
    ...
    fig.savefig(f"/home/koe3/Bioinformatics/Proposed/SHCL_v3/MLP/{data}/{ver}/Learning_Curve/[{date}_{num}]_[{experiment}]_Learning_Curve.png")
    plt.clf()
    plt.close(fig)
    
def plot_performance_evaluation(data, ver, date, num, experiment, train_auc_list, valid_auc_list):
    fig = plt.figure()
    # ax = fig.add_subplot(111)
    plt.plot(train_auc_list, color="#DC143C", label="train")
    plt.plot(valid_auc_list, color="#1E90FF", label="valid")
    plt.xlabel("epoch")
    plt.ylabel("AUC")
    plt.legend()
    plt.title(f"[{num}_{experiment}]_AUC_Plot")
    os.makedirs(f"/home/koe3/Bioinformatics/Proposed/SHCL_v3/MLP/{data}/{ver}/Evaluation_Plot/", exist_ok=True)
    fig.savefig(f"/home/koe3/Bioinformatics/Proposed/SHCL_v3/MLP/{data}/{ver}/Evaluation_Plot/[{date}_{num}]_[{experiment}]_AUC_Plot.png")
    plt.clf()
    plt.close(fig)
