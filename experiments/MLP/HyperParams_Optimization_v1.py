...
def fit(config):
    ...
    # net_hparams = [In_Nodes, [395,256, 128, 128], 1, "he_normal", "tanh", config['ENC_DR'], 0.]
    # ### 0-input_nodes, 1-hidden_nodes, 2-output_nodes, 3-initializer, 4-activation, 5-dropout
    net_hparams = [train_x_embeddings.shape[1], [128, 64, args.dim], 1, args.init, args.act, config['DR']]
    ### 0-optimizer, 1-lr, 2-lr_factor, 3-lr_patience, 4-weight_decay, 5-lr_decay_rate, 6-lr_decay_epochs
    optim_hparams = [config['OPT'], config['LR'], 0.95, 10, config['WD']]
    ...
    for epoch in range(1, experim_hparms[0] + 1):
        net.train()
        train_pred = net(train_x_embeddings)
        ...
        net.eval()
        with torch.no_grad():
            valid_pred = net(valid_x_embeddings)
    ...
    net.eval()
    with torch.no_grad():
        test_pred = net(test_x_embeddings)
        test_auc = auc(test_y, test_pred, test_w)

    print(f"trial_id: {num} \n opt_test_AUC: {opt_test_auc}, \t\t final_test_AUC: {test_auc}")

    train.report({"Training loss": train_loss, "Validation loss": valid_loss, "Training AUC": train_auc, "Validation AUC": valid_auc})
...
config = {
    ...
    # 'FAC': tune.choice([0.95, 0.99]),
    'WD': tune.uniform(0., 0.1)
    # 'LRD': tune.choice([0.9, 0.7, 0.5, 0.3, 0.1]),
    # 'LRDE': tune.choice([50, 100, 250, 500, 750, 1000]),
}
...
search_alg = OptunaSearch(metric=["Validation loss", "Validation AUC"], mode=["min", "max"])
os.makedirs(f"/home/koe3/Bioinformatics/Proposed/SHCL_v3/MLP/{data}/{ver}/", exist_ok=True)
...
result = tune.run(
    ...
    storage_path=f"/home/koe3/Bioinformatics/Proposed/SHCL_v3/MLP/{data}/{ver}/Hyperparameters/"
)
