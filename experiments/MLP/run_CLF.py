...
if __name__ == "__main__":
    ...
    net_hparams = [256, [128, 64, args.dim], 1, args.init, args.act, args.dr]
    ### 0-optimizer, 1-lr, 2-lr_factor, 3-lr_patience, 4-weight_decay, 5-lr_decay_rate, 6-lr_decay_epochs
    ...
    record = open(f"/home/koe3/Bioinformatics/Proposed/SHCL_v3/MLP/{data}/{ver}/[{date}_{num}]F-SHCSL_Result.txt", 'a+')
    ...
    # Get Train Data, Train Label, Valid Data and Valid Label
        ...
        with torch.no_grad():
            ...
            test_auc_list.append(opt_test_auc)
            pd.DataFrame(np.concatenate(([opt_test_pred.cpu().detach().numpy().ravel()], [testLabel.values.ravel()]), axis=0).T).to_csv(f"/home/koe3/Bioinformatics/Proposed/SHCL_v3/MLP/{data}/{ver}/Pred_Truth/[{date}_{num}]_[{experiment}]F-SHCSL_Pred_Truth.csv", index=False)

        record = open(f"/home/koe3/Bioinformatics/Proposed/SHCL_v3/MLP/{data}/{ver}/[{date}_{num}]F-SHCSL_Result.txt", 'a+')
        record.write("Best Test AUC: %.3f\r\n" % (opt_test_auc))
        record.close()
